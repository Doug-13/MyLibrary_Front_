...
const Statiscs = () => {
  ...
  const screenWidth = Dimensions.get('window').width;
  const [showAllBooksList, setShowAllBooksList] = useState(false);
  const [showLoanedBooksList, setShowLoanedBooksList] = useState(false); // compat
  const [allBooks, setAllBooks] = useState([]);
  const [loanedBooks, setLoanedBooks] = useState([]);
  const [loading, setLoading] = useState(true);
  const [bookStats, setBookStats] = useState({
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      (Array.isArray(data) ? data : []).forEach((book) => {
        totalBooks++;
        if (book.status === 'Lido') readBooks++;
        if (book.status === 'em progresso' || book.status === 'lendo') inProgressBooks++;
        if (book.status === 'não lido') unreadBooks++;
  ...
  const pieChartData = useMemo(() =>
    Object.keys(bookStats.genres).map((genre, idx) => ({
      name: genre,
      books: bookStats.genres[genre],
      color: colorForIndex(idx),
  ...
  [bookStats.genres, theme.textSecondary]);
  ...
  const chartConfig = useMemo(() => ({
    ...
    decimalPlaces: 0,
    color: (opacity = 1) => `rgba(78,140,255,${opacity})`, // barras
    labelColor: (opacity = 1) => isDark ? `rgba(255,255,255,${opacity})` : `rgba(45,52,54,${opacity})`,
    barPercentage: 0.6,
  }), [theme.card, isDark]);

  const pieConfig = useMemo(() => ({
    color: (opacity = 1) => isDark ? `rgba(255,255,255,${opacity})` : `rgba(45,52,54,${opacity})`,
    backgroundGradientFromOpacity: 0,
    backgroundGradientToOpacity: 0,
  }), [isDark]);

  const handleLoanPress = (userMongoId) => navigation.navigate('LoanBooksList', { userMongoId });

  if (loading) {
    return (
      <View style={[styles.loadingContainer, { backgroundColor: theme.bg }]}>
        <LottieView source={require('../../../assets/animation2.json')} autoPlay loop style={{ width: 200, height: 200 }} />
        <Text style={[styles.loadingText, { color: theme.textSecondary }]}>Carregando estatísticas, aguarde...</Text>
      </View>
    );
  }
    ...
    <FlatList
        ...
        <>
          ...
          <View style={[styles.hero, { backgroundColor: theme.primary, borderBottomColor: '#F6E68B' }]}>
            <View style={styles.heroRow}>
              ...
              <Text style={[styles.heroTitle, { color: theme.text }]}>Estatísticas de Livros</Text>
              <View style={{ width: 26 }} />
            </View>
          ...
          <Card style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
            <Text style={[styles.cardTitle, { color: theme.text }]}>Total de Livros</Text>
            <Text style={[styles.info, { color: theme.textSecondary }]}>
              Você possui um total de <Text style={[styles.infoStrong, { color: theme.text }]}>{bookStats.totalBooks}</Text> livros cadastrados.
            </Text>

            <Pressable
              style={[styles.actionButton, styles.primaryButton, { backgroundColor: theme.primary, borderColor: '#E9CC16' }]}
              onPress={() => setShowAllBooksList((s) => !s)}
            >
          ...
          {/* Livros emprestados */}
          <Card style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
            <Text style={[styles.cardTitle, { color: theme.text }]}>Livros Emprestados</Text>
            <Text style={[styles.info, { color: theme.textSecondary }]}>
              Total de livros emprestados: <Text style={[styles.infoStrong, { color: theme.text }]}>{bookStats.loanedBooks}</Text>
            </Text>

            <Pressable
              style={[styles.actionButton, styles.secondaryButton, { backgroundColor: theme.secondary, borderColor: '#3B79E6' }]}
              onPress={() => handleLoanPress(userMongoId)}
            >
              <Text style={[styles.actionButtonText, { color: '#fff' }]}>Lista de Livros Emprestados</Text>
            </Pressable>
          </Card>
        </>
      }
      ...
      renderItem={({ item }) => (
        <Card style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
          ...
          <Text style={[styles.bookDetails, { color: theme.textSecondary }]} numberOfLines={1}>Autor: {item.author}</Text>
          <Text style={[styles.bookDetails, { color: theme.textSecondary }]}>Status: {item.status}</Text>

          {!!item.activeLoans?.length && (
            <>
              <Text style={[styles.sectionLabel, { color: theme.error }]}>Empréstimos Pendentes</Text>
              {item.activeLoans.map((loan, index) => (
                <View key={index} style={[styles.loanItem, { borderLeftColor: theme.secondary }]}>
                  <Text style={[styles.bookDetails, { color: theme.textSecondary }]}>Emprestado para {loan.borrowerName}</Text>
                  <Text style={[styles.bookDetails, { color: theme.textSecondary }]}>Data de Empréstimo: {loan.loan_date}</Text>
                </View>
              ))}
            </>
          )}
          ...
          {!!item.returnedLoans?.length && (
            <>
              <Text style={[styles.sectionLabel, { color: theme.text }]}>Histórico de Empréstimos</Text>
              {item.returnedLoans.map((loan, index) => (
                <View key={index} style={[styles.loanItem, { borderLeftColor: theme.secondary }]}>
                  <Text style={[styles.bookDetails, { color: theme.textSecondary }]}>Emprestado para {loan.borrowerName}</Text>
                  <Text style={[styles.bookDetails, { color: theme.textSecondary }]}>Data de Empréstimo: {loan.loan_date}</Text>
                  <Text style={[styles.bookDetails, { color: theme.textSecondary }]}>Data de Devolução: {loan.return_date}</Text>
                </View>
              ))}
            </>
          )}
      ...
      ListFooterComponent={
        <>
          {/* Status dos livros */}
          <Card style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
            <Text style={[styles.cardTitle, { color: theme.text }]}>Status dos Livros</Text>
            <BarChart
              data={barChartData}
              width={screenWidth - 32}
              height={220}
          ...
          {/* Gêneros */}
          <Card style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
            <Text style={[styles.cardTitle, { color: theme.text }]}>Gêneros de Livros</Text>
            <PieChart
              data={pieChartData}
              width={screenWidth - 32}
              height={240}
...
const styles = StyleSheet.create({
  ...
  bookDetails: { fontSize: 13, marginTop: 2 },
  sectionLabel: {
    marginTop: 10,
    fontSize: 12,
    fontWeight: '800',
