...
const InfoRow = ({ icon, label, value, styles, t }) => (
  <View style={styles.infoRow}>
    <View style={styles.infoIcon}>
...
const Chip = ({ icon, text, tone = 'default', styles, onPrimary }) => (
  <View style={[styles.chip, tone === 'dark' && styles.chipDark]}>
    {!!icon && <Ionicons name={icon} size={12} color={tone === 'dark' ? '#fff' : onPrimary} style={{ marginRight: 6 }} />}
    <Text style={[styles.chipTxt, tone === 'dark' && styles.chipTxtDark]}>{text}</Text>
  </View>
);
...
export default function ProfilePage() {
  ...
  const onPrimary = '#111827'; // alto contraste sobre o amarelo
  ...
  return (
    <SafeAreaView style={styles.screen}>
      ...
      <View style={styles.topBar}>
        <TouchableOpacity style={styles.iconBtn} onPress={() => navigation.goBack()} accessibilityLabel="Voltar">
          <Ionicons name="chevron-back" size={22} color={onPrimary} />
        </TouchableOpacity>
        <Text style={styles.topTitle}>Meu Perfil</Text>
        <TouchableOpacity
        ...
        >
          <Ionicons name="pencil" size={18} color={onPrimary} />
        </TouchableOpacity>
      </View>
      ...
      >
        ...
        <View style={styles.cardHeader}>
          ...
          <View style={styles.chipsRow}>
            {userData?.role?.isReader ? <Chip icon="book" text="Leitor" tone="dark" styles={styles} onPrimary={onPrimary} /> : null}
            {userData?.role?.isWriter ? <Chip icon="pencil" text="Escritor" tone="dark" styles={styles} onPrimary={onPrimary} /> : null}
            <Chip icon="lock-closed" text={translateVisibility(userData?.visibilidade_biblioteca)} styles={styles} onPrimary={onPrimary} />
          </View>
          ...
          <TouchableOpacity style={styles.cta} onPress={handleEditProfile}>
            <Ionicons name="create" size={16} color={onPrimary} />
            <Text style={styles.ctaTxt}>Editar perfil</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.card}>
          <Text style={styles.sectionTitle}>Detalhes</Text>
          <InfoRow icon="call" label="Telefone" value={userData?.telefone} styles={styles} t={theme} />
          <InfoRow
            icon="calendar"
            label="Data de nascimento"
            value={
            ...
            t={theme}
          />
          <InfoRow icon="pricetags" label="Gêneros favoritos" value={genresText} styles={styles} t={theme} />
          <InfoRow
            icon="eye"
            label="Visibilidade da biblioteca"
            value={translateVisibility(userData?.visibilidade_biblioteca)}
            styles={styles}
            t={theme}
          />
        </View>
...
function mkStyles(t) {
  ...
  return StyleSheet.create({
    ...
    editBtn: { backgroundColor: '#FDE68A' },
    topTitle: { flex: 1, textAlign: 'center', fontSize: 18, color: onPrimary, fontWeight: '800' },
    ...
    name: { fontSize: 20, fontWeight: '800', color: t.text, marginTop: 10, textAlign: 'center' },
    chipsRow: { flexDirection: 'row', gap: 8, marginTop: 8, flexWrap: 'wrap', justifyContent: 'center' },
    chip: {
      flexDirection: 'row',
      alignItems: 'center',
    ...
    chipDark: { backgroundColor: '#111827', borderColor: '#111827' },
    chipTxt: { color: onPrimary, fontSize: 12, fontWeight: '800' },
    chipTxtDark: { color: '#fff' },

    bio: { marginTop: 8, color: t.textSecondary, textAlign: 'center' },
    ...
    ctaTxt: { color: onPrimary, fontWeight: '800' },
    ...
    infoLabel: { width: 150, color: t.textSecondary, fontWeight: '700' },
    infoValue: { flex: 1, color: t.text, fontWeight: '600' },
  });
}
