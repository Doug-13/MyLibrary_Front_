...
const BooksView = () => {
  ...
  // Recarrega ao focar e quando timestamp muda
  useFocusEffect(
    React.useCallback(() => {
      fetchBookDetails();
      setShowFullDescription(false);
  ...
  if (error) {
    return (
      <SafeAreaView style={[styles.root, { backgroundColor: theme.bg, alignItems: 'center', justifyContent: 'center' }]}>
        <Text style={{ color: theme.error }}>{error}</Text>
        <TouchableOpacity
          style={[
            styles.btn,
            { backgroundColor: theme.secondary, borderWidth: 1, borderColor: '#3B79E6', marginTop: 12 },
          ]}
  ...
  if (!bookDetails) {
    return (
      <SafeAreaView style={[styles.root, { backgroundColor: theme.bg, alignItems: 'center', justifyContent: 'center' }]}>
        <Text style={{ color: theme.textSecondary }}>Nenhum dado do livro.</Text>
      </SafeAreaView>
    );
  }
  ...
  const coverBg  = isDark ? '#1B1B1B' : '#F1F5F9';

  return (
    <SafeAreaView style={[styles.root, { backgroundColor: theme.bg }]}>
      {/* AppBar */}
      <View style={[styles.hero, { backgroundColor: theme.primary, borderBottomColor: '#F6E68B' }]}>
        <View style={styles.heroRow}>
          <TouchableOpacity onPress={() => navigation.goBack()} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
            <Icon name="arrow-back" size={26} color={theme.text} />
          </TouchableOpacity>
          <Text style={[styles.heroTitle, { color: theme.text }]}>Detalhes do Livro</Text>
          <TouchableOpacity onPress={handleEditPress} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
          <Image
            ...
            style={[styles.bookImage, { backgroundColor: coverBg }]}
          />
          ...
          <View style={styles.metaRow}>
            {!!bookDetails.genre && (
              <View style={[styles.chip, { backgroundColor: softTile, borderColor: theme.border }]}>
                <Icon name="category" size={14} color={theme.text} />
            ...
            {!!bookDetails.publisher && (
              <View style={[styles.chip, { backgroundColor: softTile, borderColor: theme.border }]}>
                <Icon name="apartment" size={14} color={theme.text} />
                <Text style={[styles.chipText, { color: theme.text }]} numberOfLines={1}>
                  {bookDetails.publisher}
                </Text>
              </View>
        ...
        <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
          ...
          {bookDetails.description && bookDetails.description.length > 300 && (
            <TouchableOpacity onPress={() => setShowFullDescription(!showFullDescription)} style={styles.linkBtn}>
              <Text style={[styles.linkBtnText, { color: theme.secondary }]}>
        ...
        {isBookLoaned && (
          <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
            <Text style={[styles.sectionTitle, { color: theme.text }]}>Empréstimo (Pendente)</Text>
            {pendingLoans.map((loan) => (
              <View key={loan._id} style={styles.loanRow}>
                <Icon name="person" size={16} color={theme.textSecondary} />
                <Text style={[styles.loanText, { color: theme.textSecondary }]}>
                  Solicitante: {loan.borrowerName}
                </Text>
                <Text style={[styles.loanText, { color: theme.textSecondary }]}>
                  Empréstimo: {new Date(loan.loanDate).toLocaleDateString()}
                </Text>
                <Text style={[styles.loanText, { color: theme.textSecondary }]}>
                  Devolução: {new Date(loan.returnDate).toLocaleDateString()}
                </Text>
                <View style={styles.badgeWarn}>
                  <Text style={styles.badgeWarnTxt}>{loan.status}</Text>
                </View>
                <TouchableOpacity
                  style={[styles.btn, { backgroundColor: '#FF9800', borderWidth: 1, borderColor: '#E58D00', marginTop: 8 }]}
                  onPress={() => { setSelectedLoanId(loan._id); setModalVisible(true); }}
                >
                  <Text style={styles.btnText}>Registrar devolução</Text>
                </TouchableOpacity>
        ...
        {/* Card: status de leitura */}
        <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
          <Text style={[styles.sectionTitle, { color: theme.text }]}>Status de leitura</Text>
          <View style={styles.radioGroup}>
            {['Lido', 'lendo', 'não lido'].map((opt) => (
              <View key={opt} style={styles.radioItem}>
                <RadioButton
          ...
          <TouchableOpacity
            style={[styles.btn, { backgroundColor: theme.secondary, borderWidth: 1, borderColor: '#3B79E6', marginTop: 10 }]}
      ...
      <TouchableOpacity
        style={[
          styles.fab,
          { backgroundColor: theme.primary, borderColor: '#E9CC16' },
        ]}
        onPress={handleLoanOrReturn}
        activeOpacity={0.9}
      >
        <Icon name={isBookLoaned ? 'assignment-turned-in' : 'volunteer-activism'} size={22} color="#111827" />
        <Text style={[styles.fabText, { color: '#111827' }]}>
          {isBookLoaned ? 'Devolução' : 'Emprestar'}
        </Text>
      </TouchableOpacity>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={[styles.modalCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
            <Text style={[styles.modalTitle, { color: theme.text }]}>Confirmar devolução?</Text>
            <View style={styles.modalRow}>
              <TouchableOpacity style={[styles.btn, { backgroundColor: theme.secondary, borderWidth: 1, borderColor: '#3B79E6', flex: 1 }]} onPress={handleReturn}>
                <Text style={styles.btnText}>Sim</Text>
              </TouchableOpacity>
              <View style={{ width: 10 }} />
              <TouchableOpacity style={[styles.btn, { backgroundColor: theme.card, borderWidth: 1, borderColor: theme.border, flex: 1 }]} onPress={() => setModalVisible(false)}>
                <Text style={{ fontWeight: '800', color: theme.text }}>Cancelar</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  bookTitle: { fontSize: 20, fontWeight: '800', textAlign: 'center' },
  bookAuthor: { fontSize: 14, textAlign: 'center', marginTop: 4 },
  ...
  bookDescription: { fontSize: 14, lineHeight: 22, textAlign: 'justify' },
  linkBtn: { marginTop: 6, alignSelf: 'center' },
  linkBtnText: { fontWeight: '800' },

  // Empréstimo
  loanRow: { marginTop: 4 },
  loanText: { fontSize: 13, marginTop: 4 },
  badgeWarn: {
    alignSelf: 'flex-start',
    backgroundColor: '#FFF1D6',
    borderWidth: 1,
  ...
  loadingText: { marginTop: 10, fontSize: 16, textAlign: 'center', fontWeight: '500' },
  ...
  modalTitle: { fontSize: 16, fontWeight: '800', textAlign: 'center', marginBottom: 12 },
  modalRow: { flexDirection: 'row', marginTop: 6 },
});
