...
const AddBooks = () => {
  ...
  return (
    <SafeAreaView style={styles.root}>
      ...
      <View style={styles.appbar}>
        ...
        <Text style={styles.appbarTitle}>Adicionar Livro</Text>
        <TouchableOpacity onPress={handleSubmit(onSubmit)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
          <Icon name="check" size={24} color={theme.text} />
        </TouchableOpacity>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.card}>
          {image ? (
            ...
            <View style={[styles.cover, { alignItems: 'center', justifyContent: 'center' }]}>
              <Text style={{ color: theme.label, fontWeight: '700' }}>Sem imagem</Text>
            </View>
          )}
          {!!title && <Text style={styles.title} numberOfLines={2}>{title}</Text>}
          {!!authors && <Text style={styles.subtitle} numberOfLines={1}>{authors}</Text>}
          <View style={styles.metaRow}>
            ...
            {!!publishedDate && (
              <View style={styles.chip}>
                <Icon name="event" size={14} color={theme.text} />
                <Text style={styles.chipTxt}>{publishedDate}</Text>
              </View>
            )}
            {pageCount ? (
              <View style={styles.chip}>
                <Icon name="menu-book" size={14} color={theme.text} />
                <Text style={styles.chipTxt}>{pageCount} págs</Text>
              </View>
            ) : null}
          </View>
        ...
        <View style={styles.card}>
          <View style={styles.rowBetween}>
            ...
            >
              <Icon name="help-outline" size={20} color={theme.textSecondary} />
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.radioRowWrap}>
            {['Lido', 'Lendo', 'Não lido'].map((opt) => (
              <TouchableOpacity key={opt} style={styles.radioItem} onPress={() => {
                ...
                <RadioButton
                  value={opt}
                  status={statusUi === opt ? 'checked' : 'unchecked'}
                  color={theme.primary}
                  uncheckedColor={theme.label}
                />
                <Text style={styles.radioTxt}>{opt}</Text>
      ...
      {isLoading && (
        <View style={styles.loadingOverlay}>
          <View style={styles.loadingCard}>
            ...
            <Text style={{ marginTop: 8, color: theme.textSecondary }}>Salvando…</Text>
          </View>
        </View>
      )}
    </SafeAreaView>
  ...
  const styles = useMemo(() => {
    // fundos sutis no dark
    const coverBg = isDark ? '#1B1B1B' : '#F1F5F9';
    const chipBg = isDark ? 'rgba(255,255,255,0.06)' : '#F8FAFC';
    ...
    return StyleSheet.create({
      ...
      title: { fontSize: 18, fontWeight: '800', textAlign: 'center', color: theme.text },
      subtitle: { fontSize: 13, color: theme.textSecondary, textAlign: 'center', marginTop: 4 },
      metaRow: { flexDirection: 'row', gap: 8, justifyContent: 'center', flexWrap: 'wrap', marginTop: 8 },
      chip: {
        flexDirection: 'row',
        alignItems: 'center',
      ...
      chipTxt: { color: theme.text, fontWeight: '700', fontSize: 12 },
      smallMuted: { fontSize: 12, color: theme.textSecondary, textAlign: 'center', marginTop: 8 },
      desc: { marginTop: 10, color: theme.textSecondary, fontSize: 13, lineHeight: 20, textAlign: 'justify' },
      ...
      primaryBtn: { backgroundColor: theme.primary, borderWidth: 1, borderColor: '#E9CC16' },
      btnTxt: { color: theme.text, fontWeight: '800' },
