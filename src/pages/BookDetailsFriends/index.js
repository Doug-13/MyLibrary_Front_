...
export default function BookDetailsFriends({ route }) {
  ...
  const statusStyle = useMemo(
    () => ({ backgroundColor: book?.status === 'available' ? theme.success : theme.error }),
    [book?.status, theme.success, theme.error]
  );
  ...
  // fundos sutis que mudam no dark
  const neutralTile = isDark ? 'rgba(255,255,255,0.06)' : '#F1F5F9';
  ...
  return (
    <SafeAreaView style={[styles.root, { backgroundColor: theme.bg }]}>
      ...
      <View
        style={[
          styles.appbar,
          { backgroundColor: theme.primary, borderBottomColor: '#F6E68B' },
        ]}
      >
        <TouchableOpacity
          onPress={() => {
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        >
          <Image source={imageSource} style={[styles.cover, { backgroundColor: coverBg }]} />
          ...
          <View style={styles.badgeRow}>
            <View
              style={[
                styles.badge,
                { backgroundColor: neutralTile, borderWidth: 1, borderColor: theme.border },
              ]}
            >
              <Icon name="category" size={14} color={theme.text} />
              <Text style={[styles.badgeTxt, { color: theme.text }]} numberOfLines={1}>
                {book?.genre || 'Gênero não informado'}
              </Text>
            </View>
            <View style={[styles.badge, statusStyle]}>
        ...
        >
          ...
          {descriptionText.length > 160 && (
            <Pressable onPress={() => setIsExpanded((v) => !v)}>
              <Text style={[styles.readMore, { color: theme.secondary }]}>
        ...
        >
          <Text style={[styles.sectionTitle, { color: theme.text }]}>Empréstimos</Text>
...
const styles = StyleSheet.create({
  ...
  appbarTitle: { flex: 1, textAlign: 'center', fontSize: 16, fontWeight: '800' },
  ...
  readMore: { marginTop: 6, fontSize: 13, fontWeight: '700' },
  ...
  primaryBtnDisabled: { opacity: 0.6 },
  primaryBtnTxt: { fontWeight: '800', color: '#111827' }, // texto escuro no botão amarelo
});
