...
import React, { useState, useContext, useEffect, useRef, useCallback, memo, useMemo } from 'react';
import {
  View,
  ScrollView,
  StyleSheet,
...
// ---------- Subcomponentes ----------
const Chip = memo(({ label, selected, onPress, icon, iconColor, selectedIconColor, styles }) => (
  <TouchableOpacity onPress={onPress} style={[styles.chip, selected && styles.chipActive]} activeOpacity={0.9}>
    {icon ? (
      <Ionicons
        name={icon}
        size={14}
    ...
    <SafeAreaView style={styles.screen}>
      ...
      <View style={styles.topBar}>
        ...
        <TouchableOpacity
          ...
          disabled={isSaving}
        >
          <Ionicons name="checkmark" size={22} color={onPrimary} />
        </TouchableOpacity>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.avatarWrap}>
          <TouchableOpacity activeOpacity={0.9} onPress={handleImagePick} style={styles.avatarBtn}>
            <FastImage
              source={
                foto_perfil
                  ? { uri: foto_perfil, priority: FastImage.priority.normal }
            ...
            />
            <View style={styles.editBadge}>
              <Ionicons name="camera" size={14} color={onPrimary} />
            </View>
          </TouchableOpacity>
          <Text style={styles.changePhotoText}>Alterar foto</Text>
        </View>
        ...
        {/* Dados básicos */}
        <Section title="Informações básicas" styles={styles}>
          <Controller
            control={control}
            name="name"
            rules={{ required: 'Informe seu nome completo' }}
        ...
        <Section title="" styles={styles}>
          <Controller
            control={control}
            name="phone"
            rules={{ minLength: { value: 8, message: 'Telefone inválido' } }}
            render={({ field: { onChange, value } }) => (
              <TextInput
                label="Telefone"
                mode="outlined"
        ...
        {/* Sobre mim */}
        <Section title="Sobre mim" styles={styles}>
          <TextInput
            label="Escreva um pouco sobre você"
            value={aboutMe}
            onChangeText={setAboutMe}
        ...
        {/* Tipo de perfil */}
        <Section title="Tipo de perfil" styles={styles}>
          <View style={styles.chipsWrap}>
            <Chip
              styles={styles}
              label="Leitor"
        ...
        {/* Visibilidade da biblioteca */}
        <Section title="Visibilidade da biblioteca" styles={styles}>
          <View style={styles.chipsWrap}>
            <Chip
              styles={styles}
              label="Pública"
...
function mkStyles(t, onPrimary) {
  return StyleSheet.create({
    ...
    saveBtn: { backgroundColor: '#FDE68A' }, // realce leve; pode ficar assim em ambos os temas
    ...
    changePhotoText: { marginTop: 8, color: t.textSecondary, fontWeight: '600' },
    ...
    helpTxt: { color: t.textSecondary, fontSize: 12 },

    input: { marginTop: 8, backgroundColor: t.card },
    ...
    dateTxt: { color: t.text, fontWeight: '600' },
    ...
    chipTxt: { color: t.text, fontWeight: '700' },
    chipTxtActive: { color: onPrimary },
