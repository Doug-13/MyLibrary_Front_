...
const NotificationItem = memo(function NotificationItem({
  ...
  const softTile = isDark ? 'rgba(255,255,255,0.06)' : '#F1F5F9';
  const unreadBg = isDark ? 'rgba(78,140,255,0.12)' : '#F8FAFF';
  const unreadBorder = isDark ? 'rgba(78,140,255,0.45)' : '#93C5FD';
  ...
  return (
    <Pressable
      style={[
        ...
        isUnread && [{ backgroundColor: unreadBg, borderColor: unreadBorder }],
      ]}
      onPress={onPress}
    >
      {/* Avatar */}
      <View style={styles.avatarWrap}>
        <Image
          source={item.foto_perfil ? { uri: item.foto_perfil } : require('../../../assets/perfilLendo.png')}
          style={[styles.avatar, { backgroundColor: softTile }]}
        />
        {isUnread && (
          <View
            style={[
              styles.unreadDot,
              { backgroundColor: theme.secondary, borderColor: theme.card },
            ]}
          />
        )}
      </View>
      ...
      <View style={{ flex: 1 }}>
        <View style={styles.titleRow}>
          <View
            style={[
              styles.typeChip,
              { backgroundColor: softTile, borderColor: theme.border },
            ]}
          >
            {isBook ? (
              <Ionicons name="book" size={12} color={theme.text} />
            ) : isLoan ? (
              <MaterialIcons name="swap-horiz" size={14} color={theme.text} />
            ) : (
              <Ionicons name="person-add" size={12} color={theme.text} />
            )}
            <Text style={[styles.typeChipTxt, { color: theme.text }]}>
              {isBook ? 'Livro' : isLoan ? 'Empréstimo' : 'Seguidor'}
            </Text>
          </View>

          <Text style={[styles.timeTxt, { color: theme.textSecondary }]}>{timeAgo(item.created_at)}</Text>
        </View>

        <Text style={[styles.titleTxt, { color: theme.text }]} numberOfLines={2}>
          {isFollower
            ? `${item.nome_completo} começou a te seguir!`
            : isLoan
              ? `${item.nome_completo} solicitou emprestado "${item.titleBook ?? 'seu livro'}".`
        ...
        {item.titleBook ? (
          <Text style={[styles.subtitleTxt, { color: theme.textSecondary }]} numberOfLines={1}>
        ...
        {isLoan && (
          <View style={styles.buttonsRow}>
            {/* Botões (se quiser reativar, já herdará o tema) */}
            {/* <TouchableOpacity
      ...
      <View style={styles.trailing}>
        {(isBook || isLoan) && item.imageBook ? (
          <Image source={{ uri: item.imageBook }} style={[styles.bookThumb, { backgroundColor: softTile }]} />
        ) : null}
...
export default function ChatScreen() {
  ...
  const renderItem = useCallback(
    ...
    [followStatus, handleFollow, handleUnfollow, markAsRead, navigation, userMongoId, handleAcceptLoan, handleDeclineLoan, theme, isDark]
  );

  const ListEmpty = () => (
    <View style={styles.emptyWrap}>
      <Image source={require('../../../assets/emptyLibrary.jpg')} style={styles.emptyImg} resizeMode="contain" />
      <Text style={[styles.emptyTitle, { color: theme.text }]}>Nada por aqui</Text>
      <Text style={[styles.emptyMsg, { color: theme.textSecondary }]}>Sem mensagens recentes nos últimos 7 dias.</Text>
    </View>
  );
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.bg }]}>
      ...
      <View style={[styles.topBar, { backgroundColor: theme.primary }]}>
        <TouchableOpacity style={[styles.iconBtn, { backgroundColor: isDark ? 'rgba(0,0,0,0.15)' : 'rgba(0,0,0,0.05)' }]} onPress={() => navigation.goBack()} accessibilityLabel="Voltar">
      ...
      <View style={styles.filters}>
        ...
        >
          <Text style={[styles.chipTxt, { color: theme.text }, filter === 'all' && { color: '#111827' }]}>Todos</Text>
          <View style={[styles.badge, { backgroundColor: theme.border }, filter === 'all' && styles.badgeActive]}>
            <Text style={styles.badgeTxt}>{notifications.length}</Text>
          </View>
        </Pressable>
        ...
        <Pressable
          style={[
            styles.chip,
            { backgroundColor: theme.card, borderColor: theme.border },
            filter === 'unread' && { backgroundColor: theme.primary, borderColor: theme.primary },
          ]}
          onPress={() => setFilter('unread')}
        >
          <Text style={[styles.chipTxt, { color: theme.text }, filter === 'unread' && { color: '#111827' }]}>Não lidas</Text>
          <View style={[styles.badge, { backgroundColor: theme.border }, filter === 'unread' && styles.badgeActive]}>
            <Text style={styles.badgeTxt}>{notifications.filter(n => n.status === 'unread').length}</Text>
          </View>
        </Pressable>
        ...
        <Pressable
          style={[
            styles.chip,
            { backgroundColor: theme.card, borderColor: theme.border },
            filter === 'followers' && { backgroundColor: theme.primary, borderColor: theme.primary },
          ]}
          onPress={() => setFilter('followers')}
        >
          <Text style={[styles.chipTxt, { color: theme.text }, filter === 'followers' && { color: '#111827' }]}>Seguidores</Text>
          <View style={[styles.badge, { backgroundColor: theme.border }, filter === 'followers' && styles.badgeActive]}>
            <Text style={styles.badgeTxt}>{notifications.filter(n => n.messageType === 'newFollower').length}</Text>
          </View>
        </Pressable>
        ...
        <Pressable
          style={[
            styles.chip,
            { backgroundColor: theme.card, borderColor: theme.border },
            filter === 'books' && { backgroundColor: theme.primary, borderColor: theme.primary },
          ]}
          onPress={() => setFilter('books')}
        >
          <Text style={[styles.chipTxt, { color: theme.text }, filter === 'books' && { color: '#111827' }]}>Livros</Text>
          <View style={[styles.badge, { backgroundColor: theme.border }, filter === 'books' && styles.badgeActive]}>
            <Text style={styles.badgeTxt}>{notifications.filter(n => n.messageType === 'bookAdded').length}</Text>
          </View>
        </Pressable>
        ...
        <Pressable
          style={[
            styles.chip,
            { backgroundColor: theme.card, borderColor: theme.border },
            filter === 'loans' && { backgroundColor: theme.primary, borderColor: theme.primary },
          ]}
          onPress={() => setFilter('loans')}
        >
          <Text style={[styles.chipTxt, { color: theme.text }, filter === 'loans' && { color: '#111827' }]}>Empréstimos</Text>
          <View style={[styles.badge, { backgroundColor: theme.border }, filter === 'loans' && styles.badgeActive]}>
            <Text style={styles.badgeTxt}>{notifications.filter(n => n.messageType === 'loanRequest').length}</Text>
          </View>
        </Pressable>
      </View>
...
const styles = StyleSheet.create({
  ...
  filters: { flexDirection: 'row', gap: 8, paddingHorizontal: 16, paddingVertical: 8, flexWrap: 'wrap' },
  chip: { flexDirection: 'row', alignItems: 'center', gap: 8, borderWidth: 1, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 999 },
  chipActive: { }, // estilos de cor aplicados inline pelo tema
  chipTxt: { fontWeight: '600' },
  chipTxtActive: { }, // cor inline
  ...
  titleRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  typeChip: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 999, borderWidth: 1 },
  typeChipTxt: { fontSize: 11, fontWeight: '700' },
  timeTxt: { fontSize: 11 },

  titleTxt: { fontSize: 14, fontWeight: '800', marginTop: 6 },
  subtitleTxt: { fontSize: 12, marginTop: 2 },

  // Linha de botões "Aceitar/Recusar" para empréstimo
  buttonsRow: {
    flexDirection: 'row',
    gap: 8,
    marginTop: 10,
  ...
  trailing: { alignItems: 'flex-end', gap: 8, marginLeft: 8 },
  bookThumb: { width: 48, height: 64, borderRadius: 6 },
