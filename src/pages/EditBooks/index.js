...
const EditBooks = ({ route }) => {
  ...
  const handleImagePicker = async () => {
    Alert.alert(
      ...
      'Você quer tirar uma foto ou escolher da galeria?',
      [
        { text: 'Tirar Foto', onPress: () => Alert.alert('Dica', 'Ative o ImagePicker para usar a câmera.') },
        { text: 'Escolher da Galeria', onPress: () => Alert.alert('Dica', 'Ative o ImagePicker para usar a galeria.') },
        { text: 'Cancelar', style: 'cancel' },
      ],
      { cancelable: true }
    );
  ...
  const renderRating = () => (
    <View style={styles.ratingContainer}>
      <Text style={[styles.label, { color: theme.textSecondary }]}>Avaliação do Livro</Text>
      <View style={styles.stars}>
        {[1, 2, 3, 4, 5].map((star) => (
          <TouchableOpacity key={star} onPress={() => setRating(star)}>
            <Icon
  ...
  const tile = isDark ? 'rgba(255,255,255,0.06)' : '#F1F5F9';
    ...
    <SafeAreaView style={[styles.root, { backgroundColor: theme.bg }]}>
      ...
      <View style={[styles.hero, { backgroundColor: theme.primary, borderBottomColor: '#F6E68B' }]}>
        <View style={styles.heroRow}>
          ...
          <Text style={[styles.heroTitle, { color: theme.text }]}>{bookId ? 'Editar Livro' : 'Novo Livro'}</Text>
          <TouchableOpacity onPress={handleSubmit(handleSaveChanges)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
            <Icon name="save" size={24} color={theme.text} />
          </TouchableOpacity>
        </View>
      </View>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scroll}>
          ...
          <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
            <View style={{ alignItems: 'center' }}>
              {image ? (
                <Image source={{ uri: image }} style={[styles.bookImage, { backgroundColor: tile }]} />
              ) : (
                <View style={[styles.placeholder, { backgroundColor: tile, borderColor: theme.border }]}>
                  <Text style={{ color: theme.textSecondary }}>Imagem não disponível</Text>
                </View>
              )}

              <TouchableOpacity
                onPress={handleImagePicker}
                style={[styles.imagePicker, { backgroundColor: theme.primary, borderColor: '#E9CC16' }]}
              >
                <Icon name="photo-camera" size={20} color={theme.text} />
                <Text style={[styles.imageText, { color: theme.text }]}> Nova Capa</Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
            ...
            <Text style={[styles.label, { color: theme.textSecondary }]}>Autor</Text>
            <Controller
              control={control}
              name="author"
              render={({ field: { onChange, value } }) => (
            ...
            <Text style={[styles.label, { color: theme.textSecondary }]}>Editora</Text>
            <Controller
              control={control}
              name="publisher"
              render={({ field: { onChange, value } }) => (
            ...
            <Text style={[styles.label, { color: theme.textSecondary }]}>Número de páginas</Text>
            <Controller
              control={control}
              name="page_count"
              render={({ field: { onChange, value } }) => (
            ...
            <Controller
              ...
              render={({ field: { onChange } }) => (
                <View>
                  <View style={[styles.pickerWrap, { borderColor: theme.border, backgroundColor: theme.card }]}>
                    <Picker
                      selectedValue={selectedGenre}
                      onValueChange={(v) => {
                        setSelectedGenre(v);
            ...
            <Text style={[styles.label, { color: theme.textSecondary }]}>Data de publicação</Text>
            <Controller
              control={control}
              name="publishedDate"
              render={({ field: { value } }) => (
          ...
          {/* Opções */}
          <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
            <Text style={[styles.sectionTitle, { color: theme.text }]}>Opções</Text>

            <Text style={[styles.label, { color: theme.textSecondary }]}>Visibilidade</Text>
            <View style={styles.radioGroup}>
              <View style={styles.radioItem}>
                <RadioButton value="public" status={isPublic ? 'checked' : 'unchecked'} onPress={() => setIsPublic(true)} />
                <Text style={[styles.radioLabel, { color: theme.text }]}>Público</Text>
              </View>
              <View style={styles.radioItem}>
                <RadioButton value="private" status={!isPublic ? 'checked' : 'unchecked'} onPress={() => setIsPublic(false)} />
                <Text style={[styles.radioLabel, { color: theme.text }]}>Privado</Text>
              </View>
            </View>

            <Text style={[styles.label, { color: theme.textSecondary }]}>Status de leitura</Text>
            <View style={styles.radioGroup}>
              {['lido', 'lendo', 'não lido'].map((opt) => (
                <View key={opt} style={styles.radioItem}>
                  <RadioButton value={opt} status={isLearn === opt ? 'checked' : 'unchecked'} onPress={() => setIsLearn(opt)} />
                  <Text style={[styles.radioLabel, { color: theme.text }]}>{opt[0].toUpperCase() + opt.slice(1)}</Text>
                </View>
              ))}
            </View>
          ...
          <View style={styles.actionsRow}>
            <TouchableOpacity
              style={[styles.btn, styles.primaryBtn, { backgroundColor: theme.secondary, borderColor: '#3B79E6' }]}
            ...
            {!!bookId && (
              <TouchableOpacity
                style={[styles.btn, styles.dangerBtn, { backgroundColor: theme.error, borderColor: isDark ? '#8A1621' : '#B51F2D' }]}
