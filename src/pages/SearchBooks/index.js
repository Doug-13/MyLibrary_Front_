...
const SearchBooks = () => {
  ...
  const toGBooksQuery = useCallback((raw) => {
    if (!raw) return '';
    const onlyDigits = raw.replace(/[^\dxX]/g, '');
    if (onlyDigits.length === 10 || onlyDigits.length === 13) return `isbn:${onlyDigits}`;
    return encodeURIComponent(raw);
  }, []);
  ...
  const renderRatingStars = (rating) => {
    ...
    return (
      <View style={styles.ratingContainer}>
        {Array.from({ length: full }).map((_, i) => (
          <Icon key={`f-${i}`} name="star" size={14} color={starColor} />
        ))}
        {half && <Icon name="star-half" size={14} color={starColor} />}
        {Array.from({ length: empty }).map((_, i) => (
          <Icon key={`e-${i}`} name="star-outline" size={14} color={starColor} />
        ))}
      </View>
    );
  };
      ...
      <SafeAreaView style={styles.root}>
        ...
        <View style={styles.hero}>
          <View style={styles.heroRow}>
            ...
            >
              <Icon name="arrow-back" size={26} color={onPrimary} />
            </TouchableOpacity>
            <Text style={styles.heroTitle}>Buscar Livros</Text>
            <View style={{ width: 26 }} />
          </View>
        ...
        {books.length > 0 && !loading && (
          <TouchableOpacity
            ...
            activeOpacity={0.9}
          >
            <Icon name="library-add" size={24} color={onPrimary} />
          </TouchableOpacity>
        )}
      </SafeAreaView>
    </TouchableWithoutFeedback>
...
function mkStyles(t) {
  ...
  return StyleSheet.create({
    ...
    hero: {
      ...
      paddingBottom: 12,
      borderBottomWidth: 1,
      borderBottomColor: '#F6E68B', // leve highlight sob o primary
    },
    heroRow: {
      flexDirection: 'row',
      alignItems: 'center',
    ...
    heroTitle: { fontSize: 18, fontWeight: '800', color: onPrimary },
    ...
    searchInput: { flex: 1, color: t.text, paddingVertical: 0, fontSize: 14 },
    ...
    title: { fontSize: 13, fontWeight: '700', color: t.text, textAlign: 'center' },
    author: { fontSize: 11, color: t.textSecondary, textAlign: 'center', marginTop: 2 },

    ratingContainer: { flexDirection: 'row', marginTop: 4 },

    // Footer / Empty
    emptyWrap: { alignItems: 'center', marginTop: 32, paddingHorizontal: 24 },
    emptyText: { color: t.textSecondary, textAlign: 'center' },
